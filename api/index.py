...
# Setup Jinja2 Templates with robust path resolution
possible_dirs = [
    os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "templates"),
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "templates"),
    os.path.join(os.getcwd(), "templates"),
    os.path.join(os.getcwd(), "Nehal-Dashboard-Latest-Private", "templates"),
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "templates"),
    "/var/task/templates",
    "/var/task/Nehal-Dashboard-Latest-Private/templates",
...
async def list_projects():
    client = get_supabase()
    if not client:
        raise HTTPException(status_code=500, detail="Supabase not configured. Please set SUPABASE_URL and SUPABASE_KEY in Vercel Environment Variables.")
    try:
        res = client.table("projects").select("*").order("created_at", desc=True).execute()
        return res.data or []
    except Exception as e:
